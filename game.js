...
function createJanusz() {
  ...
  const hairMat = toonMat(0xC9C9C9);
  ...
  // grey hair on the back/sides only (bald on top)
  const hairSide = new THREE.Mesh(
    new THREE.SphereGeometry(0.275, 16, 16, 0, Math.PI * 2, Math.PI * 0.4, Math.PI * 0.55),
    hairMat
  );
  hairSide.position.y = 0.86;
  hairSide.rotation.x = Math.PI;
  p.add(hairSide);
...
function createTractor() {
  ...
  roof.position.set(0, 2.25, 0.55);
  roof.castShadow = true;
  g.add(roof);

  const pillarGeo = new THREE.CylinderGeometry(0.055, 0.055, 1.05, 6);
  ...
  g.add(wheelRL, wheelRR, wheelFL, wheelFR);
...
function makeItemSprite(type) {
  ...
  const ctx = c.getContext('2d');
  ctx.fillStyle = type.bad ? 'rgba(228,87,46,0.18)' : 'rgba(255,255,255,0.92)';
  roundRect(ctx, 8, 8, size - 16, size - 16, 26); ctx.fill();
  ctx.strokeStyle = type.bad ? '#E4572E' : '#FFC145';
  ctx.lineWidth = 7;
...
function showSpeech(text) {
  ...
  speechTimer = setTimeout(() => speechEl.classList.remove('visible'), 2600);
}
