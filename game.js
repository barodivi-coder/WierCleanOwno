...
function createJanusz() {
  ...
  const hairMat = toonMat(0x6E6E6E);
  ...
  const bald = new THREE.Mesh(
    new THREE.SphereGeometry(0.265, 20, 20, 0, Math.PI * 2, 0, Math.PI * 0.42),
    skinMat
  );
  bald.position.y = 0.9;
  p.add(bald);
  ...
  const hairSide = new THREE.Mesh(
    new THREE.SphereGeometry(0.275, 20, 20, 0, Math.PI * 2, Math.PI * 0.42, Math.PI * 0.4),
    hairMat
  );
...
function createTractor() {
  ...
  roof.position.set(0, 2.85, 0.55);
  roof.castShadow = true;
  g.add(roof);

  const pillarGeo = new THREE.CylinderGeometry(0.055, 0.055, 1.73, 6);
  [[-0.85, -0.15], [0.85, -0.15], [-0.85, 1.25], [0.85, 1.25]].forEach(([x, z]) => {
    const pillar = new THREE.Mesh(pillarGeo, blackMat);
    pillar.position.set(x, 1.915, z);
    g.add(pillar);
  });
